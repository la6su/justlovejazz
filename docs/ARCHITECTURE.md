...
- `SplashCube.update()` exits on unrelated demand frames when authored jelly,
  face, opener and blend/material state are settled; those explicit dirty and
  reaction boundaries remain the only paths that advance cube work.
...
- `SceneCoordinator.init()` clears derived config/range caches before rebuilding
  ...
  contract.
- `SceneCoordinator` receives reduced-motion changes from `Experience` and
  caches the policy used by its frame-path predicates; coordinator updates no
  longer query media state repeatedly while deciding which owners to advance.
...
- `CinematicNav` uses its synchronized motion snapshot for programmatic scroll
  behavior, keeping DOM navigation and scene motion on the same policy boundary.
...
- The same active update pass snapshots `SplashCube` and Contact Cyprus owners
  once before their update/visibility branches; lazy getters remain the source
  of truth across subsequent route frames.
- Carousel visibility and particle drift share one `SectionGroups.groups`
  snapshot per active update; group identity remains owner-stable within the
  synchronous frame.
...
- `GroundPlane` also caches the section pair and eased `t`, avoiding redundant
  color interpolation work on settled transform passes without bypassing theme
  invalidation or changed transitions.
...
- `StateBus.set()` preserves absent-channel creation and animation ownership but
  skips an identical numeric value, keeping settled section-opacity passes from
  performing redundant channel-map writes.
...
- `Cursor.drawCircle()` reuses one owner-scoped ring-point buffer and color
  view. The active pointer path remains allocation-free; typed theme changes
  refresh the cached values and request exactly one settled redraw before the
  existing idle guard resumes.
...
- `initMenuLifecycle()` returns the disposer for the app-owned menu bindings;
  `useJlzPage` invokes it before route-root unmount. It owns preview hover/focus
  listeners only; the flat menu uses direct anchors, while accordion markup and
  delayed visibility RAFs are absent. Route transitions use the shared
  navigation handler.
