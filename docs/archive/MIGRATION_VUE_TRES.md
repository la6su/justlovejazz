...
### DrawTrail TSL uniform isolation — 2026-08-26

`DrawTrail` now creates `uTime`, `uVelocity` and `uEnergy` per instance and
builds its TSL color/opacity closures from that owner-local set. Multiple
route-stage trails cannot overwrite one another's GPU signal state, while the
existing geometry/material disposal and allocation-free update path remain
unchanged.

Acceptance: `DrawTrail.lifecycle.test.ts` verifies independent uniform state;
