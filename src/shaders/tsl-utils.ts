 ...
 *
 * CRITICAL: The hash functions use dot(uv, vec2(...)) to collapse vec2 → float,
 * matching the original GLSL implementation. Using mul() instead of dot()
 * produces vec2, and color.add(vec2) is a vec4+vec2 type mismatch that
 * silently fails on WebGPU (WGSL strict typing → async validation error →
 * black frame, no JS exception) while appearing to work on WebGL2 (GLSL
 * drivers are lenient about vector size mismatches).
 */
export const applyProfessionalGrain = (
  ...
  strength: TSLNode = float(0.03)
    ...
    // dot(vec2, vec2) → float (not mul which stays vec2)
    const noise1 = dot(uv, vec2(12.9898, 78.233))
        .add(time)
        .sin()
        .mul(43758.5453)
        .fract()

    const noise2 = dot(uv, vec2(34.123, 12.456))
        .add(time.mul(0.5))
        .cos()
        .mul(12345.678)
        .fract()

    // grain is float — color.add(float) broadcasts correctly to vec4
