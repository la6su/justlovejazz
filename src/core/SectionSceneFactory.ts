...
// Uses ONLY built-in three.js materials (MeshBasicMaterial, MeshStandardMaterial,
// GridHelper, PointsMaterial, LineBasicMaterial) — no ShaderMaterial, no TSL.
// Works on BOTH WebGPU and WebGL without any adapter code.
...
function makeGradientBG(topColor: number, bottomColor: number): THREE.Mesh {
  ...
  for (let i = 0; i < positions.count; i++) {
    const y = positions.getY(i) / 50 // -1 to 1
    const h = y * 0.5 + 0.5 // 0 to 1
...
// ── Studio floor with grid (GridHelper, no shader) ──
function makeStudioFloor(color: number, opacity: number, y: number): THREE.GridHelper {
...
// ── Animated slashes (simple opacity planes, no shader) ──
function makeSlashes(): THREE.Group {
  const group = new THREE.Group()
  const planeGeo = new THREE.PlaneGeometry(0.3, 4)
  for (let i = 0; i < 6; i++) {
    const mat = new THREE.MeshBasicMaterial({
      color: 0x3a5a8a,
      transparent: true,
    ...
    plane.position.set((i - 2.5) * 1.5, 0.5, -3)
...
// ── Scrolling road (LineSegments, no shader) ──
function makeRoad(): THREE.Group {
  const group = new THREE.Group()
  const lineCount = 10
  for (let i = 0; i < lineCount; i++) {
    const geo = new THREE.BufferGeometry().setFromPoints([
      new THREE.Vector3(-3, 0, -i * 2),
      new THREE.Vector3(3, 0, -i * 2),
    ])
    const mat = new THREE.LineBasicMaterial({
      color: 0x1a2a4a,
      transparent: true,
      opacity: 0.2 * (1 - i / lineCount),
    })
    const line = new THREE.Line(geo, mat)
    line.name = `road-line-${i}`
    group.add(line)
  }
  return group
}

// ── Floating transparents ──
...
export class SectionSceneFactory {
  static createStep01(): THREE.Group {
    ...
    bg.name = 'step01-bg'
    group.add(bg)
    const floor = makeStudioFloor(0x2a3a5a, 0.15, -2)
    floor.name = 'step01-floor'
    group.add(floor)
    const transparents = makeFloatingTransparents()
    transparents.name = 'step01-transparents'
    group.add(transparents)
    const particles = makeParticles(80, new THREE.Vector3(12, 6, 6), 0x4a6fa5, 0.04, 0.4)
  ...
  static createStep02(): THREE.Group {
    ...
    for (let i = 0; i < 32; i++) {
      ...
      dot.position.set(Math.cos(angle) * ringRadius, Math.sin(angle) * ringRadius * 0.3, 0)
      dot.name = `step02-ring-dot-${i}`
      group.add(dot)
    }
    const floor = makeStudioFloor(0x2a3a5a, 0.1, -2)
    floor.name = 'step02-floor'
    group.add(floor)
    const glow = new THREE.Mesh(
      new THREE.SphereGeometry(0.5, 24, 24),
      new THREE.MeshBasicMaterial({ color: 0x5a8ac5, transparent: true, opacity: 0.4 })
    )
    glow.name = 'step02-glow'
    group.add(glow)
    const particles = makeParticles(40, new THREE.Vector3(8, 4, 4), 0x3a5a8a, 0.03, 0.3)
  ...
  static createStep05(): THREE.Group {
    ...
    const floor = makeStudioFloor(0x2a3a5a, 0.1, -2.5)
    floor.name = 'step05-floor'
    group.add(floor)
    const particles = makeParticles(60, new THREE.Vector3(14, 6, 5), 0x3a5a7a, 0.035, 0.3)
  ...
  static createStep06(): THREE.Group {
    ...
    group.add(road)
    const floor = makeStudioFloor(0x2a3a5a, 0.08, -1.8)
    floor.name = 'step06-floor'
    group.add(floor)
    const bg = makeGradientBG(0x080810, 0x030305)
    bg.name = 'step06-bg'
    group.add(bg)
    const particles = makeParticles(30, new THREE.Vector3(10, 5, 4), 0x2a3a5a, 0.025, 0.25)
