...
const disposeWorkCards = vi.hoisted(() => vi.fn())
vi.mock('uikit', () => ({ default: { update: uiKitUpdate } }))
vi.mock('../UI/WorkCards', () => ({ disposeWorkCards }))

import { useJlzPage } from '../app/useJlzPage'

describe('useJlzPage idle UIkit lifecycle', () => {
  it('disposes the WorkCards registry when the route owner unmounts', async () => {
    disposeWorkCards.mockClear()

    const Root = defineComponent({
      setup() {
        const root = ref<HTMLElement | null>(null)
        useJlzPage('works', () => root.value)
    ...
    app.mount(host)
    await nextTick()
    expect(disposeWorkCards).toHaveBeenCalledTimes(1)

    app.unmount()
    expect(disposeWorkCards).toHaveBeenCalledTimes(2)
  })
